...
import {createAction, createReducer, ActionCreator, Action} from 'redux-act';
...
describe('createAction', () => {
  ...
  function testActionCreator<P>(actionCreator: ActionCreator<P, Action<P>>) {
  ...
  it('should dispatch actions', function () {
    reducer = createReducer({
      [firstAction.toString()]: (state, payload)=> {
        state.first += payload;
        return state;
      },
      [secondAction.toString()]: (state, payload)=> {
  ...
  it('should chain correctly using assignTo', function () {
    ...
    actions[action.toString()] = (state)=> state + 1;
  ...
  it('should support fake async action', function (done) {
    ...
    const reducer = createReducer({
      [start.toString()]: (state) => ({ result: state.result, running: true }),
      [success.toString()]: (state, result) => ({ running: false, result })
    }, {
      running: false,
      result: false
    });
